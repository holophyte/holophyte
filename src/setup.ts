...
async function generateApiKey(
  ...
  try {
    ...
    const rawKey = await httpClient.action(api.apiKeys.generate, {
...
async function main() {
  ...
  try {
    ...
    let keyExpiredAt: number | undefined;

    if (localKey) {
      // Validate local key against server
      ...
      try {
        ...
        // Network error — can't validate, treat as unknown
      }

      if (!keyIsActive) {
        // Check if there's an expired CLI-generated key for context
        try {
          const keys = await httpClient.query(api.apiKeys.list, {});
          const cliKey = keys.find(
            (k) =>
          ...
          if (cliKey) keyExpiredAt = cliKey.expiresAt;
        } catch {
          // Can't list keys — proceed without context
        }
      }
    }
    ...
    if (keyIsActive) {
      // Find the key's creation date for display
      let createdLabel = '';
      try {
        const keys = await httpClient.query(api.apiKeys.list, {});
        const cliKey = keys.find(
        ...
        if (cliKey) {
          createdLabel = `, created ${new Date(cliKey._creationTime).toLocaleDateString()}`;
        }
      } catch {
        // Non-critical — just skip the date
      }
      info(
        `You already have an active MCP API key ("MCP (auto-generated by CLI)"${createdLabel}).`,
      );
      process.stdout.write(
    ...
    } else if (keyExpiredAt) {
      console.log(
        `\x1b[33m⚠\x1b[0m  Your MCP API key has expired (expired ${new Date(keyExpiredAt).toLocaleDateString()}).`,
      );
      process.stdout.write(
        'Generate a new API key for the Holophyte MCP? [Y/n]: ',
      );
