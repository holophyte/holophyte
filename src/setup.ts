...
async function main() {
  ...
  try {
    ...
    if (shouldGenerate) {
      // Revoke existing CLI-generated keys before creating a new one
      try {
        const keys = await httpClient.query(api.apiKeys.list, {});
        for (const key of keys) {
          if (
            key.name === 'MCP (auto-generated by CLI)' &&
            key.revokedAt === undefined
          ) {
            await httpClient.mutation(api.apiKeys.revoke, { keyId: key._id });
          }
        }
      } catch {
        // Best-effort — proceed even if revocation fails
      }
