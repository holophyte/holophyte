...
- **WS message types remain 4** (`event`, `permission`, `status`, `error`). The `messageQueued` state in Phase 2 is frontend-only — derived from the `POST /api/sessions/:id/message` response, not from a WS message. The tab bar in Phase 3 can use the same approach: track queued message state locally per session rather than waiting for a new WS type.
