...
function nextConfigured(harnesses: HarnessInfo[], from: number, delta: number): number {
  let i = from;
  while (true) {
    i += delta;
...
export function NewSessionModal({ harnesses, candidates, onSubmit, onCancel }: NewSessionModalProps) {
  const dims = useTerminalDimensions();
  const [step, setStep] = useState<1 | 2>(1);
  const [harnessIdx, setHarnessIdx] = useState(() => firstConfigured(harnesses));
  ...
  const filtered = useMemo(
    () => candidates.filter((c) => fuzzyMatch(filter, c.path) || fuzzyMatch(filter, c.label)),
    [candidates, filter],
  );
  const hi = filtered.length === 0 ? 0 : Math.min(highlight, filtered.length - 1);
  ...
  const live = useRef({ step, harnessIdx, harness, freeMode, freePath, filtered, hi });
  useEffect(() => {
    live.current = { step, harnessIdx, harness, freeMode, freePath, filtered, hi };
  });
        ...
        <>
          ...
          {freeMode ? (
            <box flexDirection="row">
              ...
              <input focused flexGrow={1} onInput={setFreePath} placeholder="/absolute/or/relative/path" />
            </box>
          ) : (
            <>
              <box flexDirection="row">
                <text fg="green">{'> '}</text>
                <input focused flexGrow={1} onInput={setFilter} placeholder="type to filter" />
              </box>
              <text>{' '}</text>
              {filtered.length === 0 ? (
                <text>
                  <span attributes={DIM}>no matches — tab for free-text path</span>
                </text>
              ) : (
                filtered.slice(0, MAX_ROWS).map((c, i) => (
                  <text key={c.path}>
                    <span>{i === hi ? '› ' : '  '}</span>
                    <span attributes={i === hi ? BOLD : undefined}>{c.label}</span>
                    {c.annotation !== undefined ? <span fg="yellow"> {c.annotation}</span> : null}
                  </text>
                ))
              )}
            </>
